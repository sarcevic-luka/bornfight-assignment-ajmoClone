...
private extension DiscoverLayout {
  func generateDiscoverLayout() -> UICollectionViewLayout {
    UICollectionViewCompositionalLayout { [weak self] sectionIndex, _ -> NSCollectionLayoutSection? in
      ...
      switch sectionType {
      case .promo:
        return self?.generatePromoLayout()
      case .news:
        return self?.generateNewsLayout()
      case .venue:
        return self?.generateVenuesLayout()
      case .venueTag(let items):
        return self?.generateTagsLayout(with: items.count)
      case .venueCategory(_, let items):
        return self?.generateTagsLayout(with: items.count)
      }
    }
  }
